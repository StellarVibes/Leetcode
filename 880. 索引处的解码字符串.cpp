// 思路
// 1.计算总长度
//     从左到右遍历字符串s,根据字符类型更新解码后的字符串总长度len
//     如果是数字d,则len *= d(表示当前字符串重复d次)
//     如果是字母,则len += 1
// 2.反向定位第k个字符
//     从右向左遍历字符串s,逐步缩小len并调整k 
//     k %= len,将k映射到当前周期内的等效位置
//     如果k == 0且当前字符是字母,说明这个字母就是第k个字符,直接返回
//     如果是数字,则len /= d,回退到重复前的长度
//     如果是字母,则len -= 1
